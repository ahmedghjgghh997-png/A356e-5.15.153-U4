...
build_kernel() {
    ...
    echo "9. البحث عن Image في جميع المواقع الممكنة"
    find . -name "Image" -o -name "Image.gz" -o -name "Image.lz4" 2>/dev/null
    echo "محتويات arch/arm64/boot:"
    ls -la arch/arm64/boot/ 2>/dev/null || echo "المجلد غير موجود"
    echo "محتويات out/arch/arm64/boot (إن وجد):"
    ls -la out/arch/arm64/boot/ 2>/dev/null || echo "لا يوجد مجلد out"
    echo "آخر 100 سطر من build.log (الأخطاء فقط):"
    grep -i "error\|fail\|undefined\|missing" build.log | tail -20
